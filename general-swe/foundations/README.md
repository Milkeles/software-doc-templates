...
Do not sell it as faster review. A template does not make a reviewer quicker; it changes what arrives. Li and colleagues, *IEEE Transactions on Software Engineering* 2022, measured projects before and after they adopted issue and PR templates and found that afterwards, incoming volume drops, discussion drops, and **resolution takes longer** — consistent with the easy and duplicate items being filtered out so the remainder is harder. A larger cross-sectional study finds the opposite on resolution time, but it compares projects rather than the same project before and after, so it cannot separate the template from everything else that differs between projects organised enough to maintain one. Either way, promise your team better-formed submissions, not a faster queue, or the first month of unchanged turnaround will be read as the template failing.
...
- Bacchelli and Bird, "Expectations, Outcomes, and Challenges of Modern Code Review", ICSE 2013, 712-721; Gonçalves, Fregnan, Baum, Schneider and Bacchelli, "Do explicit review strategies improve code review performance?", *Empirical Software Engineering* 27 (2022); Li, Yu, Wang and Wang, "To Follow or Not to Follow: Understanding Issue/Pull-Request Templates on GitHub", *IEEE Transactions on Software Engineering* (2022); Sülün and colleagues, "An Empirical Analysis of Issue Templates Usage in Large-Scale Projects on GitHub", *ACM Transactions on Software Engineering and Methodology* (2024)
